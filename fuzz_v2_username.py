...
WORDLIST_PATH = "/tmp/xato-10k.txt"
THREADS = 3
TIMEOUT = 7 

# Your Custom Logic
FAIL_STRING = "Access denied: invalid admin token"
# FAIL_SIZE_BODY_BYTES_LOWER = 10674
# FAIL_SIZE_BODY_BYTES_UPPER = 10676
FAIL_SIZES = set([
    10382,
    10675,
    10948,
...
def test_role(role, session):
    ...
    try:
        ...
        if resp.status_code not in [502, 503, 504]:
            # if (FAIL_STRING not in resp.text) and not (FAIL_SIZE_BODY_BYTES_LOWER <= len(resp.content) <= FAIL_SIZE_BODY_BYTES_UPPER):
            if (FAIL_STRING not in resp.text) and (len(resp.content) not in FAIL_SIZES):
                return {
                    "hit": True, 
                    "role": role, 
...
def main():
    ...
    print(f"[*] Filtering: '{FAIL_STRING}' & size not in [{', '.join([str(l) for l in FAIL_SIZES])}] bytes")
